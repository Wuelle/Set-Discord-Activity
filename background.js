...
// Load the actual source from local storage
chrome.storage.local.get(["source"], result => source = result.source)

// Listen for tabs being opened
chrome.runtime.onConnect.addListener(port => {
    if (port.name == "discord") {
    	// Disconnect from previously opened ports
        if (discordPort !== undefined) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    console.info(request, request.dontSave !== true, discordPort !== undefined)
